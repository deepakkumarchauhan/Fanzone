...
CGFloat const offset_HeaderStop = 200.0;
...
#pragma mark-
#pragma mark- manage Post Detail---
...
- (MBTwitterScroll *)initTableViewWithBackgound:(UIImage*)backgroundImage andDataModal:(FFProfileModal*)modal{
    ...
    [self setupView:backgroundImage  andDataModal:modal scrollHeight:0 type:MBTable];
...
{
    [self animationForScroll:100];
}
- (void) setupView:(UIImage*)backgroundImage andDataModal:(FFProfileModal*)modal scrollHeight:(CGFloat)height type:(MBType)type {
    
    
    // Header
    self.header = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, MainScreenHeight)];
    ...
    if (type == MBTable) {
        ...
        self.tableView.tableHeaderView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, self.header.frame.size.height -35)];
...
- (void) setupView:(UIImage*)backgroundImage avatarImage:(UIImage *)avatarImage titleString:(NSString *)titleString subtitleString:(NSString *)subtitleString buttonTitle:(NSString *)buttonTitle scrollHeight:(CGFloat)height type:(MBType)type {
   ...
   // [self.header addSubview:self.headerLabel];
    ...
    if (type == MBTable) {
        ...
        // TableView Header
        self.tableView.tableHeaderView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, self.header.frame.size.height-40 )];
        [self addSubview:self.tableView];
