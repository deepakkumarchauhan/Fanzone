...
-(void)updateFramesForSize:(CGSize)size
{
    ...
    if (MainScreenWidth==320) size.height = size.height-40;
...
-(void)showFromFame:(CGRect)frame inView:(UIView *)view
{
    if(!self.view)
        [self createView];
    ...
    if(!cell.clickedCharacter)
    {
        [cell setClickedCharacter:^(NSString *character) {
            ...
            if([self.delegate respondsToSelector:clickSel])
                [self.delegate emojiPopover:nil didClickedOnCharacter:character];
...
-(void)refresh
{
    ...
    if(!self->_buttons)
    {
        ...
        for(uint i = 0; i < EMOJI_NB_ITEM_IN_ROW; ++i)
        {
            ...
            [button.titleLabel setFont:[UIFont fontWithName:@"AppleColorEmoji" size:EMOJI_FONT_SIZE]];//EMOJI_FONT_SIZE
            if (MainScreenWidth==320) {
               // [button.titleLabel setFont:[UIFont fontWithName:@"AppleColorEmoji" size:25]];
            }
