...
-(void)setUpViews
{
   ...
   // [self.tableView setBackgroundColor:RGBCOLOR(20, 172, 190, 1)];
...
-(void)performSetup
{
    ...
    {
        self.textColor = RGBCOLOR(253, 234, 17, 1) ;
        self.font = AppFont(17);
...
-(void)updateFrame
{
    ...
    if (self.assignedDirection == VSDropdownDirection_Down) {
        switch (self.assignedDropdownType) {
            case DropdownType_Align: {
                frame = CGRectMake(viewFrame.origin.x, viewFrame.origin.y+viewFrame.size.height+dropdownOffset, 150, dropdownHeight);
            }
                break;
            case DropdownType_FontSize: {
                frame = CGRectMake(viewFrame.origin.x, viewFrame.origin.y+viewFrame.size.height+dropdownOffset, 60, dropdownHeight+90);
            }
                break;
            case DropdownType_None:{
        ...
        switch (self.assignedDropdownType) {
            case DropdownType_Align: {
                ...
                break;
            case DropdownType_FontSize: {
                frame = CGRectMake(viewFrame.origin.x, viewFrame.origin.y+viewFrame.size.height+dropdownOffset, 50, dropdownHeight);
            }
                break;
            case DropdownType_None:{
                frame = CGRectMake(viewFrame.origin.x, viewFrame.origin.y+viewFrame.size.height+dropdownOffset, viewFrame.size.width, dropdownHeight);
...
-(void)configureCell:(UITableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.sortedArr[index] isKindOfClass:[VSDropdownItem class]])
    {
        ...
        strImageName = ddItem.itemImage;
    }
    
    cell.textLabel.text = stringAtIndexPath;
    if(self.assignedDropdownType == DropdownType_Align) {
        [cell.textLabel setFont:AppFontBOLD(14)];
        [cell.imageView setImage:[UIImage imageNamed:strImageName]];
    }
    else if (self.assignedDropdownType == DropdownType_FontSize) {
         [cell.imageView setImage:nil];
        switch (indexPath.row) {
            case 0:
                [cell.textLabel setFont:AppFontBOLD(11)];
                break;
            case 1:
                [cell.textLabel setFont:AppFontBOLD(13)];
                break;
            case 2:
               [cell.textLabel setFont:AppFontBOLD(18)];
                break;
            case 3:
                [cell.textLabel setFont:AppFontBOLD(20)];
                break;
            case 4:
                [cell.textLabel setFont:AppFontBOLD(24)];
                break;
            case 5:
                [cell.textLabel setFont:AppFontBOLD(32)];
                break;
            default:
                break;
        }
